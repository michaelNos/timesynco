...
The final prototype validates software-clock behavior using firmware diagnostics and ROS2 logs, and additionally estimates physical PPS-to-GPIO response using a logic analyzer. The design objective is stable PPS-driven second generation, elimination of observable long-term drift, and typical sub-millisecond physical response during normal operation, not proof of absolute sub-microsecond synchronization accuracy.
...
The firmware-level evaluation can only confirm synchronization behavior at the available millisecond-level diagnostic resolution. The additional logic-analyzer measurement provides a practical PPS-to-GPIO physical response estimate, but it does not replace true hardware timestamping of the PPS edge.
