...
This chapter evaluates the implemented synchronization prototype and verifies whether the final system behavior matches the design objectives defined in the previous chapters. The evaluation focuses on DCF77 synchronization, RTK PPS-driven second generation, UART communication reliability, ROS2 logging, two-board comparison, and physical PPS-to-GPIO synchronization response.

Firmware diagnostics, UART transmission, ROS2 nodes, CSV logs, and Python analysis were used to verify software-clock behavior and communication-level timing. In addition, a logic-analyzer-based measurement was performed to estimate the physical response delay between the RTK PPS input and an STM32-generated synchronization GPIO output.

The evaluation does not prove absolute sub-microsecond synchronization accuracy. Instead, it validates stable DCF77-based time acquisition, PPS-driven second generation, reliable diagnostic communication, and typical sub-millisecond PPS-to-GPIO response during normal operation.
...
The zero-valued \texttt{RTK\_DIFF} result confirms that the software clock was driven directly from accepted PPS events at the implemented 1~ms diagnostic resolution. However, this metric does not measure the real physical response delay of the system. For this reason, an additional physical synchronization accuracy measurement is presented in Section~\ref{sec:physicalAccuracyMeasurement}.

This measurement used a logic analyzer to compare the RTK PPS input edge with a GPIO output generated by the STM32 firmware inside the PPS interrupt handling path.
...
\label{sec:physicalAccuracyMeasurement}

The \texttt{RTK\_DIFF} metric confirmed software-level PPS discipline, but it did not measure the physical delay between the external PPS edge and the STM32 response. Therefore, a separate hardware-level measurement was performed using a logic analyzer to compare the RTK PPS input with the STM32-generated synchronization GPIO output.
...
\begin{itemize} 
\item average synchronization offset, 
\item synchronization jitter, 
\item stability of the PPS-to-GPIO response over the recorded sample set,
\item occurrence of latency spikes. 
\end{itemize}
...
The logic-analyzer measurement showed that the real physical synchronization response is mainly limited by interrupt handling latency and software processing overhead. Unlike the \texttt{RTK\_DIFF} metric, this measurement directly observed the delay between the external PPS edge and the STM32 GPIO response.
...
The measurement methods used in this thesis are sufficient to verify the functional behavior of the synchronization prototype, but they do not provide absolute nanosecond-level timing characterization. The firmware-level evaluation was based on diagnostic values, UART communication, ROS2 logging, CSV files, and Python analysis scripts. This part of the evaluation provides millisecond-level visibility into the software-clock behavior and communication pipeline.

The additional logic-analyzer measurement provided practical hardware-level observation of the PPS-to-GPIO response. However, this measurement characterizes the complete implemented interrupt path rather than the intrinsic accuracy of the RTK PPS source. The measured delay includes EXTI interrupt latency, HAL processing overhead, GPIO switching delay, timer access overhead, and firmware execution jitter.

Another limitation is the non-real-time nature of the logging path. UART buffering, CP2102 USB-UART conversion, operating-system scheduling, WSL behavior, and ROS2 middleware processing can all influence message arrival time. Therefore, ROS2 arrival-time differences are useful for evaluating the communication and logging pipeline, but they cannot be interpreted as direct hardware synchronization error.
...
Finally, the tests were performed in a laboratory and ROS2-based environment rather than during complete UAV flight operation. Effects such as motor noise, vibration, airflow, battery-voltage variation, and onboard electromagnetic interference were not fully characterized.

For these reasons, the results should be interpreted as validation of stable DCF77-based time acquisition, RTK PPS-driven second generation, reliable UART/ROS2 logging, elimination of observable long-term software-clock drift, and typical sub-millisecond PPS-to-GPIO response during normal operation. They do not prove absolute sub-microsecond synchronization accuracy of the complete timing chain.
...
Overall, the final system should be understood as a functional prototype of a DCF77 and RTK PPS-based synchronization module. It demonstrates absolute time acquisition, PPS-driven second generation, stable diagnostic communication, ROS2-based evaluation, and practical hardware-level PPS-to-GPIO validation. The results verify correct system behavior, no observable long-term software-clock drift at millisecond-level firmware resolution, and typical sub-millisecond physical response during normal operation.