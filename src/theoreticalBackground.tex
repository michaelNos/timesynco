...
In the final prototype, the RTK PPS signal was detected using an external interrupt input rather than full timer input-capture mode. The interrupt was used to increment the software clock after a valid DCF77 synchronization \cite{ptbDcf77}. This implementation was sufficient to verify PPS-driven second generation and removal of long-term software-clock drift. A later logic-analyzer measurement also allowed the physical delay between the PPS input and an STM32 synchronization GPIO output to be estimated. However, because the firmware still used EXTI-based interrupt handling, the measured offset included interrupt latency, HAL overhead, GPIO switching delay, and firmware execution jitter. For future microsecond-level internal timestamping, the PPS signal should be connected to a timer input-capture channel and measured using hardware timestamps.
